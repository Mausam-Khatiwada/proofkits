import { Star, CheckCircle, Heart, Crown } from 'lucide-react';
...
export function WallOfLove({ testimonials, userPlan }: WallOfLoveProps) {
  ...
  if (!isPro) {
    return (
      <div className="bg-white rounded-2xl border border-[#EDE9FE] p-5 hover:shadow-[0_4px_20px_rgba(124,58,237,0.08)] transition-shadow duration-200">
        <h3 className="font-semibold text-sm text-gray-800">Wall of Love</h3>
        <p className="text-xs text-gray-400">Shareable showcase</p>
        <div className="mt-4 text-center py-6">
          <Heart className="w-8 h-8 text-violet-200 mx-auto" />
          <p className="text-sm font-medium text-gray-700 mt-2">Wall of Love is a Pro feature</p>
          <p className="text-xs text-gray-400 mt-1">
            Upgrade to turn approved testimonials into a dedicated showcase.
          </p>
          <a
            href={getUpgradeHref('wall_of_love')}
